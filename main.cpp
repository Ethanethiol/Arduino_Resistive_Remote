...
const int NUM_READ=512;                        //Количество опросов кнопки для фильтра шумов
const int pushDelay=20;                        //Таймаут подтверждения кода кнопки в миллисекундах. Код передается в программу после удержания кнопки в течение /pushDelay/. Можно использовать для обработки комбинаций кнопок (нужно успеть зажать комбинацию в течение pushDelay).
const int useLevel=680;                        //Уровень сигнала от пульта, выше которого считаем все кнопки отпущеными
...
int getKey() {
    ...
    //---Устанавливаем соответствие считанных сопротивлений кнопкам---
      if (Key >= 470 && Key <= 500) return (VOL_UP);
      if (Key >= 447 && Key <= 469) return (VOL_DN);
      if (Key >= 403 && Key <= 443) return (ARR_UP);
      if (Key >= 250 && Key <= 362) return (ARR_DN);
      if (Key >= 685 && Key <= 710) return (LW_UP);
      if (Key >= 650 && Key <= 684) return (LW_DN);
      if (Key >= 605 && Key <= 640) return (LW_PRESS);
      if (Key >= 520 && Key <= 570) return (LK_UP);
      if (Key >= 390 && Key <= 401) return (LK_DN);
      return (0);
}
